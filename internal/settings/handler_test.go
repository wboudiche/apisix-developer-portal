...
func doReq(h *settings.Handler, adminID int64, method, target string, body any) *httptest.ResponseRecorder {
...
func TestGetShapeAndSecretMasking(t *testing.T) {
	...
	adminID := settings.NewTestAdminID(t)
	if err := svc.Set(context.Background(), map[string]string{"SMTP_PASSWORD": "s3cret", "SMTP_HOST": "h1"}, adminID, false); err != nil {
		t.Fatalf("seed: %v", err)
	}
	h := settings.NewHandler(svc, auth.UserID)
	rec := doReq(h, adminID, http.MethodGet, "/api/admin/settings", nil)
...
func TestPutMatrix(t *testing.T) {
	...
	adminID := settings.NewTestAdminID(t)
	h := settings.NewHandler(svc, auth.UserID)
	if rec := doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"SMTP_HOST": "new"}}); rec.Code != http.StatusNoContent {
		t.Fatalf("valid PUT = %d (%s)", rec.Code, rec.Body.String())
	}
	if rec := doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"NOPE": "x"}}); rec.Code != http.StatusBadRequest {
		t.Fatalf("unknown = %d", rec.Code)
	}
	if rec := doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"JWT_SECRET": "x"}}); rec.Code != http.StatusBadRequest {
		t.Fatalf("read-only = %d", rec.Code)
	}
	rec := doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"SMTP_PORT": "nope"}})
...
func TestPutProbeFailureAndForce(t *testing.T) {
	...
	adminID := settings.NewTestAdminID(t)
	h := settings.NewHandler(svc, auth.UserID)
	rec := doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"SMTP_HOST": "bogus"}})
	...
	rec = doReq(h, adminID, http.MethodPut, "/api/admin/settings", map[string]any{"values": map[string]string{"SMTP_HOST": "bogus"}, "force": true})
...
func TestDeleteReset(t *testing.T) {
	...
	adminID := settings.NewTestAdminID(t)
	h := settings.NewHandler(svc, auth.UserID)
	_ = svc.Set(context.Background(), map[string]string{"SMTP_HOST": "x"}, adminID, false)
	if rec := doReq(h, adminID, http.MethodDelete, "/api/admin/settings/SMTP_HOST", nil); rec.Code != http.StatusNoContent {
		t.Fatalf("reset = %d", rec.Code)
	}
	if svc.Snapshot().Source["SMTP_HOST"] != "env" {
		t.Fatal("reset must fall back to env")
	}
	if rec := doReq(h, adminID, http.MethodDelete, "/api/admin/settings/NOPE", nil); rec.Code != http.StatusNotFound {
		t.Fatalf("unknown reset = %d", rec.Code)
	}
	if rec := doReq(h, adminID, http.MethodDelete, "/api/admin/settings/JWT_SECRET", nil); rec.Code != http.StatusBadRequest {
...
func TestTestEndpoint(t *testing.T) {
	...
	adminID := settings.NewTestAdminID(t)
	h := settings.NewHandler(svc, auth.UserID)
	rec := doReq(h, adminID, http.MethodPost, "/api/admin/settings/test", map[string]any{"values": map[string]string{"SMTP_HOST": "candidate"}})
