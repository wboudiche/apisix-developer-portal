...
describe('UsageChart', () => {
  it('renders the three range options and marks the active one', () => {
    render(<UsageChart state={{ status: 'loading' }} range="7d" onRange={() => {}} />)
  ...
  it('calls onRange when a different range is clicked', async () => {
    const onRange = vi.fn()
    render(<UsageChart state={{ status: 'ready', usage }} range="24h" onRange={onRange} />)
  ...
  it('renders one bar per series point when ready', () => {
    render(<UsageChart state={{ status: 'ready', usage }} range="24h" onRange={() => {}} />)
  ...
  it('shows an empty-state message when there is no traffic', () => {
    ...
    render(<UsageChart state={{ status: 'ready', usage: empty }} range="24h" onRange={() => {}} />)
  ...
  it('shows an explicit unavailable state on error', () => {
    render(<UsageChart state={{ status: 'error' }} range="24h" onRange={() => {}} />)
