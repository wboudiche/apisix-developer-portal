...
type Card = { icon: keyof typeof STAT_ICONS; label: string; unit?: string; value: (s: UsageSummary) => string }

const CARDS: Card[] = [
  { icon: 'pulse', label: "Requêtes · aujourd'hui", value: s => formatCount(s.requestsToday) },
  { icon: 'calendar', label: 'Ce mois-ci', value: s => formatCompact(s.monthToDate) },
  { icon: 'clock', label: 'Latence p95', unit: 'ms', value: s => formatMs(s.p95Ms) },
  { icon: 'alert', label: "Taux d'erreur", unit: '%', value: s => formatPercent(s.errorRate) },
]
